...
-- Schema memories
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `memories` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci ;
USE `memories` ;
...
CREATE TABLE IF NOT EXISTS `memories`.`user` (
  ...
  `join_type` VARCHAR(6) NOT NULL COMMENT '회원가입 경로(NORMAL, KAKAO, NAVER)',
  `sns_id` VARCHAR(100) NULL COMMENT 'OAuth 인증 후 인증 서버로부터 발급 받은 사용자의 아이디',
  `name` VARCHAR(15) NOT NULL COMMENT '사용자 이름',
  `adress` TEXT NOT NULL COMMENT '사용자 주소',
  `detail_adress` TEXT NULL COMMENT '사용자 상세 주소',
  `profile_image` TEXT NULL COMMENT '사용자 프로필 이미지',
  `gender` VARCHAR(5) NULL COMMENT '사용자 성별',
  `age` INT NULL COMMENT '사용자 나이 ',
  PRIMARY KEY (`user_id`),
  UNIQUE INDEX `user_id` (`user_id` ASC) VISIBLE,
  UNIQUE INDEX `sns_id` (`sns_id` ASC) VISIBLE)
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `memories`.`diary` (
  ...
  `diray_number` INT NOT NULL AUTO_INCREMENT COMMENT '일기 관리 번호',
  `write_date` VARCHAR(10) NOT NULL COMMENT '일기 작성 날짜(YYYY-MM-DD)',
  ...
  `content` TEXT NOT NULL COMMENT '일기 내용(HTML 형식)',
  PRIMARY KEY (`diray_number`),
  UNIQUE INDEX `diray_number_UNIQUE` (`diray_number` ASC) VISIBLE,
  INDEX `diary_wirte_user_idx` (`user_id` ASC) VISIBLE,
  CONSTRAINT `diary_wirte_user`
    FOREIGN KEY (`user_id`)
    REFERENCES `memories`.`user` (`user_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
...
CREATE USER 'momories' IDENTIFIED BY 'qwer1234';

GRANT ALL ON `memories`.* TO 'momories';
